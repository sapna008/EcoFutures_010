...
fetch('utils/data.json')
  ...
  .then(data => {
    data.data.forEach((product) => {
      ...
      card.classList.add('card', 'snap-start', 'flex-shrink-0', 'w-1/4', 'p-0', 'bg-white', 'shadow-lg', 'rounded-lg', 'border', 'border-transparent', 'hover:border-black', 'transition-all', 'duration-300', 'h-[450px]'); // Increased height to 450px

      const productImageUrl = `${baseUrl}${product.productAssets.path}`;
      const hoverImageUrl = `${baseUrl}${product.productAssetList[1].path}`;

      card.innerHTML = `
        <div class="image-container relative overflow-hidden h-3/5"> <!-- Adjusted to cover 60% of the card height -->
        ...
        <div class="card-details h-2/5 mt-2 text-left p-4"> <!-- Adjusted to cover 40% of the card height -->
